...
bot = discord.Bot(command_prefix=prefix, description=description)
buttons = PycordComponents(bot)
...
@bot.slash_command()
async def hello(ctx, character_name: Option(str, "Character", required=True), move: Option(str, "Move", required=True)):
    await ctx.respond(f'Character is {character_name} with move {move}')
